...
Sign in with your Atmosphere account (on a spaces-enabled PDS), paste your secret webhook URL into the Pebble app, and every capture your ring makes lands in a private space on **your own PDS**: audio-bearing deliveries become `me.byjp.pebble-index.recording` records (audio blob + transcript), transcription-only deliveries become `me.byjp.pebble-index.note` records. The app then reads your space back to list and play everything — including `me.byjp.pebble-index.reminder` records written by other Pebble Index apps sharing the space.
