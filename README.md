...
Sign in with your Atmosphere account and you get **two secret webhook URLs** to paste into the Pebble app: a **private** one that stores captures in a private space on your PDS (only you can read them — requires a spaces-enabled PDS), and a **public** one that writes them to your ordinary public atproto repo (**anyone can read those**). Audio-bearing deliveries become `me.byjp.pebble-index.recording` records (audio blob + transcript), transcription-only deliveries become `me.byjp.pebble-index.note` records. The dashboard lists everything with public/private labels and per-entry deletion — including `me.byjp.pebble-index.reminder` records written by other Pebble Index apps sharing the space. On a PDS without spaces support the private webhook is hidden and only the public one is offered.

## How it works

- **Login** is atproto OAuth (`@atproto/oauth-client-node`). The requested scope is `atproto blob?accept=audio/mp4 repo:me.byjp.pebble-index.recording repo:me.byjp.pebble-index.note include:me.byjp.pebble-index.auth` — repo grants for the public webhook, and the permission set expands to full access to your own `me.byjp.pebble-index.space` space. A PDS that rejects the spaces permission set gets a retry without it, and spaces support is probed after login, so non-spaces accounts still work (public webhook only). OAuth sessions are stored server-side (SQLite) so the webhooks can write to your PDS while you're away.
- **First login** creates a private space at `at://<your-did>/space/me.byjp.pebble-index.space/main` (`com.atproto.simplespace.createSpace` with an empty member list — nobody but you can read it). The lexicons under the `me.byjp.pebble-index.*` root (see [`lexicons/me/byjp/pebble-index/`](./lexicons/me/byjp/pebble-index)) are shared with the wider Pebble Index atproto tooling, so other apps can read and write the same space.
- **The webhook** (`POST /hook/<secret-token>`) accepts the Pebble Index multipart payload ([format docs](https://github.com/coredevices/mobileapp/blob/master/experimental/src/commonMain/kotlin/coredevices/ring/external/indexwebhook/INDEX_WEBHOOK_API.md)): an `audio` part (`audio/mp4`), a `transcription` part, or both, plus `recordedAt`/`client` fields and `X-Index-*` headers. Audio is uploaded with `com.atproto.repo.uploadBlob` and the record written with `com.atproto.space.createRecord` — a `recording` when audio is present, a `note` otherwise. Record keys are deterministic (the Pebble recording id, or a TID derived from the capture time), so the Pebble app's retry-on-next-recording behaviour is idempotent. Test events ("Send test event" in the app) are acknowledged and shown on the dashboard without writing a record.
- **The dashboard** lists your recordings, notes and reminders via one un-filtered `com.atproto.space.listRecords` call and streams audio through `/api/audio/<cid>` (`com.atproto.space.getBlob`), gated by your web session.
